...
- Une facture `PAID` porte **au moins un paiement** — pas forcément une référence (les
  espèces n'en ont pas). `syncBillPaymentInfo` refuse (`BILL_SETTLED_NEEDS_PAYMENT`) de laisser
  une facture payée sans paiement ; le chemin de sortie est `reopenBill`, qui les détache.
  `SOLDE` n'y est **pas** soumise : soldée veut dire « on ne réclame plus rien », une créance
  abandonnée, qui n'a par définition pas de règlement à montrer (voir l'en-tête de
  `syncBillPaymentInfo`). « Payée » n'exige pas non plus que les paiements couvrent le total :
  « Marquer comme payée » est justement le geste pour un règlement partiel accepté.
