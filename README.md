...
- **Layout primitives** (exported via the barrel `index.ts`): `AdminCard` (page shell) and `AdminDashboardCard` (the accent-colored count cards on `/admin`).
- **Entity search kit** — `EntitySearchCombobox` (generic debounced search-and-pick popover: request cancellation via `AbortController`, stale-results-while-loading, keyboard navigation) built on `hooks/useEntitySearch`, plus the domain wrappers `UserSearchCombobox` (optionally `assignable`-filtered for readers) and `BookSearchCombobox`. Every form base picks users/books through these — don't hand-roll a search popover.
- **Form bases** — `BookFormBackendBase`, `UserFormBackendBase`, `OrderFormBackendBase`, `AssignmentFormBackendBase`, `BillFormBackendBase`, `PaymentFormBackendBase`. The reusable engine for each entity: fields + validation, parameterized by `initialData`, `onSubmit` (returns the created/edited id), and `onSuccess`/`onDelete`. **The "new" page and the "edit" modal render the same base**, differing only in whether `initialData` is present and whether `onSubmit` creates or updates — the DRY spine of the admin. The Add/Edit wrappers live in their own files (`AddOrderFormBackend`, `EditOrderFormBackend`, `AddAssignmentFormBackend`, `EditAssignmentFormBackend`, `AddUserFormBackend`, `EditUserFormBackend`).
